...
class NLayerDiscriminator3D(nn.Module):
    ...
    """
    def __init__(self, input_nc=1, ndf=64, n_layers=3, use_actnorm=False, use_groupnorm=False,  act_tanh=False, use_blur_pool = False):
...
class LPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, kl_weight=1.0, enbale_logvar = False, use_groupnorm=False,
                 disc_num_layers=3, disc_in_channels=1, disc_ch = 64, disc_factor=1.0, disc_weight=1.0, loss_norm = 'l1',
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx, global_step, last_layer=None, split="train"):
        ...
        if optimizer_idx == 0:
            # 在 FP32 下计算 LPIPS 与重建损失，降低 AMP 带来的数值不稳定
            with torch.cuda.amp.autocast(enabled=False):
                inputs_f = inputs.float()
                recon_f = reconstructions.float()

                if self.perceptual_weight > 0:
                    p_loss = self.perceptual_loss(
                        inputs_f.transpose(2, 1).contiguous().reshape(-1, c, h, w),
                        recon_f.transpose(2, 1).contiguous().reshape(-1, c, h, w),
                    )
                ...
                if self.loss_norm == 'l1':
                    rec_loss = torch.abs(inputs_f.contiguous() - recon_f.contiguous())
                    if self.perceptual_weight > 0:
                        rec_loss = rec_loss +  p_loss
                    # 添加数值稳定性检查
                    logvar_clamped = torch.clamp(self.logvar, min=-10, max=10)
                    ...
                    rec_loss = F.mse_loss(recon_f.contiguous(), inputs_f.contiguous(), reduction='mean')
                    if self.perceptual_weight > 0:
            ...
            if split == "train":
                # 若 disc_factor 为 0（判别器未开启）则直接置零，避免梯度为 0 时自适应权重产生 NaN
                if disc_factor == 0:
                    d_weight = torch.tensor(0.0, device=inputs.device, dtype=inputs.dtype)
                    weighted_g_loss = torch.tensor(0.0, device=inputs.device, dtype=inputs.dtype)
                    loss_dict.update({f'{split}/d_weight': 0.0})
                else:
...
class LPIPSWithDiscriminator_withoutkl(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, codebook_weight=1.0, enbale_logvar = False, use_groupnorm=False,
                 disc_num_layers=3, disc_in_channels=1, disc_ch = 64, disc_factor=1.0, disc_weight=1.0, loss_norm = 'l1',
                 rec_weight = 1.0, perceptual_weight=1.0, use_actnorm=False, disc_conditional=False, act_tanh=False,
                 disc_loss="hinge", disc_loss_scale = 1.0, disc_precision="fp16", nll_loss_mean=False, warm_d_weight_steps = 0,
                 enable_2d = False, enable_3d=True, use_blur_pool=False,
    ...
    def forward(self,codebook_loss, inputs, reconstructions, optimizer_idx, global_step, last_layer=None, split="train"):
        ...
        if optimizer_idx == 0:
            ...
            loss = weighted_nll_loss + weighted_g_loss + penalty_loss + self.codebook_weight * codebook_loss.mean()
                
            loss_dict.update({f'{split}/total_loss_ae': loss.mean().clone().detach().item()})
            loss_dict.update({f'{split}/nll_loss': nll_loss.mean().clone().detach().item()})
            loss_dict.update({f'{split}/weighted_nll_loss': weighted_nll_loss.mean().clone().detach().item()})
            loss_dict.update({f'{split}/quant_loss': codebook_loss.detach().mean().item()})
            loss_dict.update({f'{split}/penalty_loss': penalty_loss.mean().clone().detach().item()})
            loss_dict.update({f'{split}/rec_loss': rec_loss.mean().clone().detach().item()})
            loss_dict.update({f'{split}/logvar': self.logvar.clone().detach().item()})
            loss_dict.update({f'{split}/g_loss': g_loss.mean().clone().detach().item()})
